	...
	class qa_html_theme_layer extends qa_html_theme_base {
		...
		function head_script(){
			...
			if(qa_opt('q2apro_onsitenotifications_enabled') && qa_is_logged_in()) {
				...
				if(qa_opt('site_theme')=='cleanstrap') {
				        $this->output('
                                        <script type="text/javascript">
                                                $(document).ready(function(){
                                                        $("#osnbox").detach().appendTo(".form-search");
                                                        //var elem = $("#osnbox").detach();
                                                        //$(".qa-nav-main-clear").prepend(elem);
                                                });
                                        </script>
                                        ');
				}
		...
		function doctype() {
			...
			if(qa_opt('q2apro_onsitenotifications_enabled') && $userid) {
				...
				if(qa_opt('site_theme')=='cleanstrap') {
					$this->content['custom'] = @$this->content['custom']. ' ' . $html;
				} else {
					$this->content['title']['suffix'] = @$this->content['title']['suffix']. ' ' . $html;
				}
			}
